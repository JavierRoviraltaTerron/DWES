    ...
    <div id="container">
      ...
      <?php
        ...
        if (isset($_POST['dni']) && ($_POST["action"] == "alta")) {
          ...
          if (mysqli_num_rows($consultaClienteExiste) > 0) {
      ?>
        <p>Ya existe un cliente con DNI <?= $_POST['dni'] ?></p>
      <?php
          } else {
            $conexion->query("INSERT INTO cliente (dni, nombre, direccion, telefono) VALUES ('$_POST[dni]','$_POST[nombre]','$_POST[direccion]','$_POST[telefono]')");
            ?>
        ...
        if (isset($_POST['dni']) && ($_POST["action"] == "baja")) {
          $conexion->query("DELETE FROM cliente WHERE dni = '" . $_POST["dni"] . "'");
          ?>
        ...
        <br/>
        <p>Número de clientes: <?= mysqli_num_rows($consulta) ?></p>
